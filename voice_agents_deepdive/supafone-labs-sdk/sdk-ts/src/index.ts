...
export interface StructuredWhisperOptions extends WhisperOptions {
  ...
  ) => SupervisorDirective | null | Promise<SupervisorDirective | null>;
}
...
const COACH_SYSTEM =
  "You are the coaching core of a supervisor for a live voice agent. Read the " +
...
function normalizeDirectiveContract(input: DirectiveContract = {}): NormalizedDirectiveContract {
  ...
  return {
    ...
    allowed_kinds: allowed.filter((kind): kind is SupervisorDirectiveKind =>
...
function normalizeStructuredDirective(
  ...
  const kind = String(raw.kind ?? "mixed") as SupervisorDirectiveKind;
...
export class SupafoneLabs {
  ...
  ): Promise<SupervisorDirective | null> {
...
class LabsAgentsNamespace {
  ...
  create(input: CreateLabsAgentRequest): Promise<CreateLabsAgentResponse> {
    return this.sm.requestSupafoneApi<CreateLabsAgentResponse>(
      "POST",
      "/api/v1/labs/agents",
      labsAgentPayload(this.withSupervisor(input)),
    );
  }
  ...
  private withSupervisor(input: CreateLabsAgentRequest): CreateLabsAgentRequest {
